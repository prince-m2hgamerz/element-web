...
export function updateMobilePage(metadata: MobileAppMetadata, deepLinkUrl: string, server: string | undefined): void {
    ...
    if (server) {
        (document.getElementById("header_title") as HTMLHeadingElement).innerText = `Join ${server} on VChat`;
    }
    ...
    step1Heading.innerHTML = step1Heading!.innerHTML.replace("VChat", metadata.name);

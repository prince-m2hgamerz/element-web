...
By default, VChat will create encrypted DM rooms if the user you are chatting with has keys uploaded on their account.
For private room creation, VChat will default to encryption on but give you the choice to opt-out.
...
By default, VChat strongly encourages (but does not require) users to set up
...
is now deprecated. VChat will check for either key, preferring

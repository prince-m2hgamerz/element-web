...
VChat will attempt to load first `config.$domain.json` and if it fails `config.json`. This mechanism allows different
...
In order for VChat to even start you will need to tell it what homeserver to connect to _by default_. Users will be
...
If both `default_server_config` and `default_server_name` are used, VChat will try to look up the connection
...
Some settings additionally support being specified at the config level to affect the user experience of your VChat Web
...
VChat supports some customisation of the user experience through various branding and theme options. While it doesn't support
...
3. `brand`: Optional name for the app. Defaults to `VChat`. This is used throughout the application in various strings/locations.
4. `permalink_prefix`: An optional URL pointing to a VChat Web deployment. For example, `https://app.element.io`. This will
   change all permalinks (via the "Share" menus) to point at the VChat Web deployment rather than `matrix.to`.
...
7. `mobile_guide_toast`: When `true` (default), users accessing the VChat Web instance from a mobile device will be prompted to
...
8. `mobile_guide_app_variant`: Optional. The mobile app that the user is prompted to download from the `/mobile_guide` page. When omitted
   the mobile guide will be configured for the new VChat X apps. Allowed values are as follows:
    1. `element`: VChat X Android/iOS.
    2. `element-classic`: VChat Classic Android/iOS.
    3. `element-pro`: VChat Pro Android/iOS.
...
1. `ios`: The URL for where to download the iOS app, such as an App Store link. When explicitly `null`, the app will assume the
   iOS app cannot be downloaded. When not provided, the default VChat app will be assumed available.
...
1. `welcome_url`: A URL to an HTML page to show as a welcome page (landing on `#/welcome`). When not specified, the default
   `welcome.html` that ships with VChat will be used instead.
...
links to your copy of VChat appear in some websites like Facebook, and indeed VChat itself. This has to be static in the HTML
...
change it, you can build VChat, but run `RIOT_OG_IMAGE_URL="http://example.com/logo.png" pnpm build`. Alternatively, you can edit
the `og:image` meta tag in `index.html` directly each time you download a new version of VChat.

## SSO setup

When VChat is deployed alongside a homeserver with SSO-only login, some options to ease the user experience might want to be set:
...
1. `logout_redirect_url`: Optional URL to redirect the user to after they have logged out. Some SSO systems support a page that the
   user can be sent to in order to log them out of that system too, making logout symmetric between VChat and the SSO system.
...
Currently, VChat uses Jitsi to offer conference calls in rooms, with an experimental VChat Call implementation in the works.
A set of defaults are applied, pointing at our Jitsi and VChat Call instances, to ensure conference calling works, however you
can point VChat at your own if you prefer.
...
3. `voip`: Optional configuration for various VoIP features. Currently can only contain a single `obey_asserted_identity` value to
   ...
   only be set in trusted environments. The option defaults to `false`. For example:
    ...
    ```
4. `widget_build_url`: Optional URL to have VChat make a request to when a user presses the voice/video call buttons in the app,
   ...
   being called in. The URL must respond with a JSON object similar to the following:
    ...
    2-person rooms, causing VChat to fall back to 1:1 VoIP, by setting the option `widget_build_url_ignore_dm` to `true`.
5. `audio_stream_url`: Optional URL to pass to Jitsi to enable live streaming. This option is considered experimental and may be removed
   at any time without notice.
6. `element_call`: Optional configuration for native group calls using VChat Call, with the following subkeys:
    - `use_exclusively`: A boolean specifying whether VChat Call should be used exclusively as the only VoIP stack in
      the app, removing the ability to start legacy 1:1 calls or Jitsi calls. Defaults to `false`.
    - `brand`: Optional name for the app. Defaults to `VChat Call`. This is
      used throughout the application in various strings/locations.
    - `guest_spa_url`: Optional URL for a VChat Call single-page app (SPA),
      for guest links. If this is set, VChat Web will expose a "join" link
      for public video rooms, which can then be shared to non-matrix users.
      The target VChat Call SPA is typically set up to use a homeserver that
...
Integration managers are embedded applications within VChat to help the user configure bots, bridges, and widgets. An integration manager
...
If you would like to use Scalar, the integration manager maintained by VChat, the following options would apply:
...
1. `analytics_owner`: the company name used in dialogs talking about analytics - this defaults to `brand`,
   and is useful when the provider of analytics is different from the provider of the VChat instance.
2. `privacy_policy_url`: URL to the privacy policy including the analytics collection policy.

## Miscellaneous

VChat supports other options which don't quite fit into other sections of this document.
...
As of VChat 1.4.0, all identity server functions are optional and you are
prompted to agree to terms before data is sent to the identity server.

VChat will check multiple sources when looking for an identity server to use in
...
If none of these sources have an identity server set, then VChat will prompt the
...
`modules`: An optional array of module paths to load at runtime. Each entry is a URL or path to a JavaScript module entry point that will be dynamically imported when VChat Web starts.
...
Each module URL is loaded using dynamic import (`import()`). The modules are loaded in order after VChat Web initializes but before the application fully starts. Modules must be accessible from the browser and should export a compatible module format that works with the [Module API](https://github.com/element-hq/element-modules/tree/main/packages/element-web-module-api).

...
If Labs is enabled in the [VChat config](config.md), you can enable some of these features by going
...
Custom themes are possible through VChat's [theme support](./theming.md), though
normally these themes need to be defined in the config for VChat. This labs flag
...
## VChat Call video rooms (`feature_element_call_video_rooms`) [In Development]

Enables support for video rooms that use VChat Call rather than Jitsi, and causes the 'New video room' option to create VChat Call video rooms rather than Jitsi ones.
...
This feature allows users to place native [MSC3401](https://github.com/matrix-org/matrix-spec-proposals/pull/3401) group calls in compatible rooms, using VChat Call.

If you're enabling this at the deployment level, you may also want to reference the docs for the `element_call` config section.

## Disable per-sender encryption for VChat Call (`feature_disable_call_per_sender_encryption`)

The default for embedded VChat Call in VChat Web is per-participant encryption.
This labs flag disables encryption for embedded VChat Call in encrypted rooms.

Under the hood this stops VChat Web from adding the `perParticipantE2EE` flag for the VChat Call widget url.
